...
struct CarePlanView: View {
    let simpleInfoData: [CarePlanSimpleInfo]
    let descriptionInfoData: [CarePlanDescriptionInfo]
    var body: some View {
        List {
            ForEach(simpleInfoData, id:\.self) { info in
                CarePlanSimpleItem(info: info)
            }
            ForEach(descriptionInfoData, id:\.self) { info in
...
struct CarePlanView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        CarePlanView(simpleInfoData: [CarePlanSimpleInfo(title: "氏名", value: "田中太朗"),
