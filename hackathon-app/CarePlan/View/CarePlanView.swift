...
struct CarePlanView: View {
    ...
    var body: some View {
        List {
            CarePlanSimpleItem(info: CarePlanSimpleInfo(title: "氏名", value: client.name))
            CarePlanSimpleItem(info: CarePlanSimpleInfo(title: "生年月日", value: client.birthDate.toJapaneseLongString))
                        ForEach(client.simpleInfoData, id:\.self) { info in
                CarePlanSimpleItem(info: info)
            }
            ForEach(client.descriptionInfoData, id:\.self) { info in
                CarePlanDescriptionInfoItem(info: info)
            }
        }
